...
func _maybe_party_food_share(item_id: String, result: Dictionary) -> void:
	...
	if combat_stats != null and combat_stats.statuses != null:
		combat_stats.statuses.apply_status("player", st, party_dur, "player")
...
func _maybe_party_skill_share(skill_id: String, result: Dictionary) -> void:
	if not bool(result.get("ok", false)):
		return
	skill_id = skill_id.strip_edges()
	if skill_id.is_empty() or skill_catalog == null:
		return
	var def: Dictionary = skill_catalog.get_skill(skill_id)
	if def.is_empty() or not bool(def.get("party_share", false)):
		return
	var st_v: Variant = def.get("status", {})
	if typeof(st_v) != TYPE_DICTIONARY or (st_v as Dictionary).is_empty():
...
func note_party_kill(npc_kind: String) -> Array:
	...
	if npc_kind.is_empty() or quest_journal == null or not _party_quest_share_eligible():
		return []
	return _party_quest_share_result(bool(quest_journal.note_kill(npc_kind)))
...
func note_party_gather(gather_id: String, qty: int = 1) -> Array:
	...
	qty = maxi(qty, 0)
	if gather_id.is_empty() or qty <= 0 or quest_journal == null or not _party_quest_share_eligible():
		return []
	var changed := false
	if quest_journal.has_method("note_gather"):
		changed = bool(quest_journal.note_gather(gather_id, qty))
	else:
		changed = bool(quest_journal.note_item_gain(gather_id, qty))
...
func note_party_fish(item_id: String = "", qty: int = 1) -> Array:
	item_id = item_id.strip_edges()
	qty = maxi(qty, 0)
	if qty <= 0 or quest_journal == null or not _party_quest_share_eligible():
		return []
	if not quest_journal.has_method("note_fish"):
		return []
	return _party_quest_share_result(bool(quest_journal.note_fish(item_id, qty)))
...
func snapshot_party() -> Dictionary:
	...
	return {
		...
		"loot_mode": party_loot_mode if party_loot_mode.strip_edges() != "" else "ffa",
...
func _party_online_same_map_count() -> int:
	...
	for m in _party_members:
		...
		if mid_map != "" and mid_map != map_pack_id:
			continue
		n += 1
	return n
...
func _party_assign_kill_loot_owner() -> String:
	if not in_party():
		return ""
	var mode := party_loot_mode.strip_edges().to_lower()
	if mode == "" or mode == "ffa" or mode == "need_greed" or mode == "roll":
		return ""
	if mode == "leader":
		return party_leader_id.strip_edges()
	if mode == "round_robin":
		var ids: Array = _party_online_same_map_ids()
		if ids.is_empty():
			return ""
		var n: int = ids.size()
		var idx: int = posmod(_party_loot_rr_index, n)
		var oid := str(ids[idx])
		_party_loot_rr_index += 1
		return oid
	return ""
...
func try_party_set_loot_mode(mode: String) -> Dictionary:
	...
	party_loot_mode = mode
	if mode == "round_robin":
		_party_loot_rr_index = maxi(_party_loot_rr_index, 0)
...
func _party_member_display_name(member_id: String) -> String:
	...
	if idx >= 0:
		var nm := str((_party_members[idx] as Dictionary).get("name", "")).strip_edges()
		if nm != "":
			return nm
...
func _start_party_loot_roll(cell: Dictionary, item_id: String, qty: int, npc_id: String, source: String, eligible: Array) -> Array:
	var actions: Array = []
	var rid := "lr_%d" % _next_loot_roll_seq
	_next_loot_roll_seq += 1
	var elig: Array = []
	for e in eligible:
	...
	_loot_rolls[rid] = {
		...
		"item_id": item_id,
		"qty": qty,
		"cell": {"x": int(cell.get("x", player_cell.x)), "y": int(cell.get("y", player_cell.y))},
		"npc_id": str(npc_id),
		"source": str(source),
		"eligible": elig,
		"choices": {},
...
func _party_member_statuses(member_id: String) -> Array:
	...
	if mid == self_id or mid == "player":
		if combat_stats != null and combat_stats.statuses != null:
			return combat_stats.statuses.snapshot_statuses("player")
		return []
...
func _party_refresh_self_member() -> bool:
	...
	if idx < 0:
		...
		_party_last_self_status_fp = st_fp
		return true
	...
	if changed or fp != _party_last_self_hp_fp or st_fp != _party_last_self_status_fp:
		_party_last_self_hp_fp = fp
		_party_last_self_status_fp = st_fp
		return true
	return false
...
func _party_clear() -> void:
	...
	party_loot_mode = "ffa"
	_party_loot_rr_index = 0
	_loot_rolls.clear()
	loot_roll_rng_fn = Callable()
	_party_last_self_hp_fp = ""
	_party_last_self_status_fp = ""
	_party_invites.clear()
	_party_summon_pending.clear()
	party_summon_auto_accept = false
...
func try_party_create() -> Dictionary:
	...
	actions.append_array(_note_achievement_counter("party", 1))
	_mark_party_poll()
	return {"ok": true, "actions": actions}
...
func try_party_invite(target: String = "") -> Dictionary:
	...
	var inv_id := "inv_%d" % _next_invite_seq
	_next_invite_seq += 1
	_party_invites.append({
		"id": inv_id,
...
func _tick_party_invites() -> Array:
	var actions: Array = []
	if _party_invites.is_empty():
		return actions
	var now: float = _party_clock()
	var keep: Array = []
	for inv_v in _party_invites:
...
func try_party_invite_respond(invite_id: String, accept: bool) -> Dictionary:
	...
	var idx := -1
	for i in range(_party_invites.size()):
		var inv: Variant = _party_invites[i]
	...
	if idx < 0:
		...
		return {"ok": false, "reason": "not_found", "actions": actions}
	var row: Dictionary = _party_invites[idx]
	_party_invites.remove_at(idx)
	...
	if not bool(row.get("outgoing", true)):
		if accept:
			...
			party_leader_id = str(stub.get("id", ""))
			actions.append(_party_update_action())
...
func snapshot_party_summon() -> Dictionary:
	if _party_summon_pending.is_empty():
		return {"active": false}
	return {
		"active": true,
		"id": str(_party_summon_pending.get("id", "")),
		"caster_id": str(_party_summon_pending.get("caster_id", "")),
		"caster_cell": (_party_summon_pending.get("caster_cell", {}) as Dictionary).duplicate(true) if typeof(_party_summon_pending.get("caster_cell", {})) == TYPE_DICTIONARY else {},
		"map_id": str(_party_summon_pending.get("map_id", "")),
		"invites": (_party_summon_pending.get("invites", {}) as Dictionary).duplicate(true) if typeof(_party_summon_pending.get("invites", {})) == TYPE_DICTIONARY else {},
		"expires_at": float(_party_summon_pending.get("expires_at", 0.0)),
	}
...
func _try_party_summon_item(item_id: String, def: Dictionary) -> Dictionary:
	...
	if inventory == null or not inventory.has_item(item_id, 1):
		...
		return {"ok": false, "reason": "missing", "actions": actions}
	if combat_stats != null and not combat_stats.is_item_ready(item_id):
	...
	var free: Array = _party_summon_list_free_cells(player_cell.x, player_cell.y, {})
	if free.is_empty():
		actions.append({"type": "system_message", "text": "没有空余位置。"})
	...
	if combat_stats != null:
		combat_stats.set_item_cooldown(item_id, cd)
	var summon_id := "psum_%d" % _next_party_summon_seq
	_next_party_summon_seq += 1
	...
	if combat_stats != null:
		expires_at = float(combat_stats.now_sec()) + PARTY_SUMMON_WINDOW_SEC
	_party_summon_pending = {
		"id": summon_id,
		"caster_id": self_id,
		"caster_cell": {"x": player_cell.x, "y": player_cell.y},
		"map_id": map_pack_id,
		"invites": invites,
	...
	actions.append({
		"type": "inventory_update",
		"items": inventory.snapshot(),
		"gold": inventory.get_gold() if inventory.has_method("get_gold") else 0,
	})
...
func _party_summon_cell_free(x: int, y: int) -> bool:
	if player_cell.x > -9990 and x == player_cell.x and y == player_cell.y:
		return false
	if map_collision != null:
		if map_collision.has_method("is_landable") and not bool(map_collision.is_landable(x, y)):
			return false
		if map_collision.has_method("is_extra_blocked") and bool(map_collision.is_extra_blocked(x, y)):
			return false
	# Occupied by remotes
	for rid in _remote_players.keys():
		var rd: Variant = _remote_players[rid]
	...
	for m in _party_members:
		...
		if typeof(mc) == TYPE_DICTIONARY:
			if int(mc.get("x", -9999)) == x and int(mc.get("y", -9999)) == y:
				return false
	if not _party_summon_pending.is_empty():
		var inv_v: Variant = _party_summon_pending.get("invites", {})
		if typeof(inv_v) == TYPE_DICTIONARY:
...
func try_party_summon_accept(member_id: String = "") -> Dictionary:
	...
	if combat_stats != null:
		var now: float = float(combat_stats.now_sec())
		if now > float(_party_summon_pending.get("expires_at", 0.0)) + 0.001:
			_party_summon_pending.clear()
			actions.append({"type": "system_message", "text": "集结已过期。"})
			return {"ok": false, "reason": "expired", "actions": actions}
	var invites_v: Variant = _party_summon_pending.get("invites", {})
	...
	if bool(inv_row.get("accepted", false)):
		...
		return {"ok": false, "reason": "already", "actions": actions}
	var cc_v: Variant = _party_summon_pending.get("caster_cell", {"x": player_cell.x, "y": player_cell.y})
	var cx: int = player_cell.x
	var cy: int = player_cell.y
	...
	_party_summon_pending["invites"] = invites
	...
	actions.append({
		...
		"summon_id": str(_party_summon_pending.get("id", "")),
		"member_id": member_id,
		"x": dest.x,
		"y": dest.y,
	})
...
func try_trade_put_item(item_id: String, qty: int = 1) -> Dictionary:
	...
	if inventory.has_method("consume_unbound"):
		consumed_ok = inventory.consume_unbound(item_id, qty)
	else:
		consumed_ok = inventory.consume(item_id, qty)
	if not consumed_ok:
...
func snapshot_friends() -> Dictionary:
	...
	return friend_list.snapshot_state(online_ids, online_names)
...
func _resolve_friend_target(name_or_id: String) -> Dictionary:
	...
	# Direct remote id
	if _remote_players.has(name_or_id):
		var rd: Dictionary = _remote_players[name_or_id]
	...
	var rid := find_remote_by_name(name_or_id)
	if rid != "":
		var rd2: Dictionary = get_remote_player(rid)
...
func try_friend_add(name_or_id: String) -> Dictionary:
	...
	var add_r: Dictionary = friend_list.try_add(fid, fname)
	if not bool(add_r.get("ok", false)):
		...
		if r2 == "full":
			actions.append({"type": "system_message", "text": "好友列表已满（最多 %d 人）。" % friend_list.max_friends()})
			return {"ok": false, "reason": "full", "actions": actions}
...
func try_friend_remove(friend_id: String) -> Dictionary:
	...
	var fname := friend_id
	var idx: int = friend_list.find_index_by_id(friend_id)
	if idx >= 0 and idx < friend_list._friends.size():
		var e: Variant = friend_list._friends[idx]
		if typeof(e) == TYPE_DICTIONARY:
			fname = str(e.get("name", friend_id))
	var rem: Dictionary = friend_list.try_remove(friend_id)
...
func _mail_inject_welcome() -> void:
	if _mail_welcome_sent:
		return
	if mailbox == null:
		mailbox = Mailbox.new()
	if mailbox.count() > 0:
		_mail_welcome_sent = true
		return
...
func _resolve_mail_recipient(to_name: String) -> Dictionary:
	...
	var self_name := _party_self_name()
	var self_id := _party_self_id()
	...
	var resolved: Dictionary = _resolve_friend_target(to_name)
...
func try_mail_send(to: String, subject: String, body: String, gold: int = 0, item_id: String = "", qty: int = 1) -> Dictionary:
	...
	if is_self and mailbox.is_full():
		actions.append({"type": "system_message", "text": "收件箱已满（最多 %d 封）。" % mailbox.max_mail()})
		return {"ok": false, "reason": "full", "actions": actions}
	if inventory == null:
		actions.append({"type": "system_message", "text": "背包不可用。"})
		return {"ok": false, "reason": "no_inv", "actions": actions}
	if gold > 0 and inventory.get_gold() < gold:
		actions.append({"type": "system_message", "text": "金币不足。"})
		return {"ok": false, "reason": "no_gold", "actions": actions}
	if not item_id.is_empty():
		...
		if inventory.has_method("can_transfer") and not inventory.can_transfer(item_id, qty):
			...
			return {"ok": false, "reason": "bound", "actions": actions}
	# Deduct attachments
	if gold > 0 and not inventory.try_spend_gold(gold):
		actions.append({"type": "system_message", "text": "金币不足。"})
		return {"ok": false, "reason": "no_gold", "actions": actions}
	if not item_id.is_empty():
		var mail_ok := false
		if inventory.has_method("consume_unbound"):
			mail_ok = inventory.consume_unbound(item_id, qty)
		else:
			mail_ok = inventory.consume(item_id, qty)
		if not mail_ok:
			if gold > 0:
	...
	if is_self:
		var add_r: Dictionary = mailbox.try_add(_party_self_name(), recip_name, subject, body, gold, attach)
		if not bool(add_r.get("ok", false)):
			# Refund
			if gold > 0:
				inventory.add_gold(gold)
...
func try_mail_read(mail_id: String) -> Dictionary:
	...
	var r: Dictionary = mailbox.try_mark_read(mail_id)
	if not bool(r.get("ok", false)):
...
func try_mail_claim(mail_id: String) -> Dictionary:
	...
	if mail_id.is_empty():
		actions.append({"type": "system_message", "text": "无效的邮件。"})
		return {"ok": false, "reason": "invalid", "actions": actions}
	if mailbox == null or inventory == null:
		actions.append({"type": "system_message", "text": "邮箱不可用。"})
		return {"ok": false, "reason": "no_box", "actions": actions}
	var mail: Dictionary = mailbox.get_mail(mail_id)
	if mail.is_empty():
		actions.append({"type": "system_message", "text": "找不到该邮件。"})
		return {"ok": false, "reason": "not_found", "actions": actions}
	...
	for it in items:
		...
		if iid.is_empty() or q <= 0:
			continue
		if not inventory.can_accept(iid, q):
	...
	for it2 in items:
		...
		if iid2.is_empty() or q2 <= 0:
			continue
		var add_r: Dictionary = inventory.try_add_item(iid2, q2)
		var added: int = int(add_r.get("added", 0))
		if added < q2:
			# Rollback gold already granted; refund partial items
			if gold > 0:
				inventory.try_spend_gold(gold)
			if added > 0:
				inventory.consume(iid2, added)
			actions.append({"type": "system_message", "text": "背包已满，无法收取附件。"})
			actions.append_array(_mail_inventory_actions())
			return {"ok": false, "reason": "bag_full", "actions": actions}
	var mark: Dictionary = mailbox.try_mark_claimed(mail_id)
...
func try_mail_delete(mail_id: String) -> Dictionary:
	...
	if mailbox == null:
		actions.append({"type": "system_message", "text": "邮箱不可用。"})
		return {"ok": false, "reason": "no_box", "actions": actions}
	var r: Dictionary = mailbox.try_delete(mail_id)
	if not bool(r.get("ok", false)):
		actions.append({"type": "system_message", "text": "找不到该邮件。"})
		return {"ok": false, "reason": str(r.get("reason", "not_found")), "actions": actions}
	actions.append(_mail_update_action())
...
func snapshot_duel() -> Dictionary:
	if not _duel.is_empty() and bool(_duel.get("active", false)):
		return {
			"active": true,
			"pending": false,
			"opponent_id": str(_duel.get("opponent_id", "")),
			"opponent_name": str(_duel.get("opponent_name", "")),
			"started_at": float(_duel.get("started_at", 0.0)),
			"ends_at": float(_duel.get("ends_at", 0.0)),
			"opponent_hp": int(_duel.get("opponent_hp", 0)),
			"opponent_hp_max": int(_duel.get("opponent_hp_max", DUEL_STUB_HP)),
		}
	if not _duel_pending.is_empty():
		return {
			"active": false,
			"pending": true,
			"opponent_id": str(_duel_pending.get("opponent_id", "")),
			"opponent_name": str(_duel_pending.get("opponent_name", "")),
			"started_at": 0.0,
			"ends_at": 0.0,
			"opponent_hp": 0,
...
func in_duel() -> bool:
	return not _duel.is_empty() and bool(_duel.get("active", false))
...
func _duel_resolve_target(target_id_or_name: String) -> Dictionary:
	...
	var rid := ""
	if _remote_players.has(key):
		rid = key
	else:
		rid = find_remote_by_name(key)
	if rid.is_empty():
...
func _duel_is_opponent(target_id: String) -> bool:
	if not in_duel():
		return false
	return str(target_id).strip_edges() == str(_duel.get("opponent_id", ""))


func _duel_seed_remote_hp(remote_id: String, hp: int, hp_max: int) -> void:
	if not _remote_players.has(remote_id):
		return
	var row: Variant = _remote_players[remote_id]
	if typeof(row) != TYPE_DICTIONARY:
		return
	var d: Dictionary = row
	d["hp"] = hp
	d["hp_max"] = hp_max
	_remote_players[remote_id] = d
...
func _duel_end(reason: String, winner: String = "") -> Array:
	...
	var actions: Array = []
	var opp_id := str(_duel.get("opponent_id", ""))
	var opp_name := str(_duel.get("opponent_name", "对手"))
...
func _tick_duel() -> Array:
	if not in_duel():
		return []
	var now: float = _party_clock()
	if now < float(_duel.get("ends_at", 0.0)):
		return []
...
func try_duel_accept() -> Dictionary:
	...
	if in_duel():
		...
		actions.append(_duel_update_action())
		return {"ok": false, "reason": "already", "actions": actions}
	if _duel_pending.is_empty():
		actions.append({"type": "system_message", "text": "没有待接受的决斗。"})
		return {"ok": false, "reason": "no_pending", "actions": actions}
	var oid := str(_duel_pending.get("opponent_id", ""))
	var oname := str(_duel_pending.get("opponent_name", oid))
	actions.append_array(_duel_start(oid, oname))
	return {"ok": true, "actions": actions}
...
func try_duel_decline() -> Dictionary:
	...
	if _duel_pending.is_empty():
		...
		return {"ok": false, "reason": "no_pending", "actions": actions}
	var oname := str(_duel_pending.get("opponent_name", "对方"))
	_duel_pending.clear()
	actions.append(_duel_update_action())
...
func try_duel_debug_pending(target_id_or_name: String) -> Dictionary:
	var actions: Array = []
	if in_duel() or not _duel_pending.is_empty():
		return {"ok": false, "reason": "busy", "actions": actions}
	var resolved: Dictionary = _duel_resolve_target(target_id_or_name)
	if not bool(resolved.get("ok", false)):
		return {"ok": false, "reason": "not_found", "actions": actions}
...
func _duel_player_atk() -> int:
	if combat_engine != null and combat_engine.has_method("_effective_atk_player"):
		return maxi(1, int(combat_engine._effective_atk_player()))
	if combat_stats != null and typeof(combat_stats.player) == TYPE_DICTIONARY:
		return maxi(1, int(combat_stats.player.get("atk", 10)))
	return 10
...
func _duel_apply_damage(amount: int) -> Array:
	...
	var hp: int = maxi(0, int(_duel.get("opponent_hp", 0)) - dealt)
	var hp_max: int = int(_duel.get("opponent_hp_max", DUEL_STUB_HP))
	_duel["opponent_hp"] = hp
	var oid := str(_duel.get("opponent_id", ""))
...
func _try_duel_attack_target(target_id: String, player_x: int, player_y: int) -> Dictionary:
	...
	if combat_engine != null and combat_engine.is_casting():
		actions.append({"type": "system_message", "text": "施法中，无法普攻。"})
		return {"ok": false, "actions": actions}
	if combat_stats != null and not combat_stats.is_attack_ready():
		actions.append({"type": "system_message", "text": "攻击冷却中。"})
		return {"ok": false, "actions": actions}
	var rd: Dictionary = get_remote_player(target_id)
	if rd.is_empty():
	...
	if _chebyshev(player_x, player_y, rx, ry) > DUEL_RANGE_CELLS:
		actions.append({"type": "system_message", "text": "目标太远。"})
		return {"ok": false, "reason": "range", "actions": actions}
	var cd: float = 0.8
	if combat_stats != null and typeof(combat_stats.player) == TYPE_DICTIONARY:
		cd = float(combat_stats.player.get("atk_speed", 0.8))
		combat_stats.set_attack_cooldown(cd)
		combat_stats.set_skill_cooldown("basic_attack", cd)
...
func _try_duel_skill_target(skill_id: String, target_id: String, player_x: int, player_y: int) -> Dictionary:
	...
	if _chebyshev(player_x, player_y, rx, ry) > range_cells:
		actions.append({"type": "system_message", "text": "目标太远。"})
		return {"ok": false, "reason": "range", "actions": actions}
	if combat_stats != null and combat_stats.has_method("is_skill_ready") and not combat_stats.is_skill_ready(skill_id):
	...
	if combat_stats != null and combat_stats.has_method("set_skill_cooldown"):
...
func in_guild() -> bool:
	return guild != null and guild.in_guild()


func _guild_self_id() -> String:
	return _party_self_id()


func _guild_self_name() -> String:
	return _party_self_name()
...
func try_guild_create(guild_name: String) -> Dictionary:
	...
	var gid := "guild_%d" % _next_guild_seq
	_next_guild_seq += 1
	var r: Dictionary = guild.try_create(gid, guild_name, _guild_self_id(), _guild_self_name())
	if not bool(r.get("ok", false)):
...
func try_guild_invite(target: String) -> Dictionary:
	...
	if guild.member_count() >= guild.max_members():
		actions.append({"type": "system_message", "text": "公会人数已满（最多 %d 人）。" % guild.max_members()})
		return {"ok": false, "reason": "full", "actions": actions}
	# Resolve: remotes / friends / known shell names / free-form stub
	var mid := ""
	var mname := ""
	var resolved: Dictionary = _resolve_friend_target(target)
	if bool(resolved.get("ok", false)):
	...
	if mid == _guild_self_id():
		...
		return {"ok": false, "reason": "self", "actions": actions}
	if guild.has_member(mid) or guild.has_member_name(mname):
		actions.append({"type": "system_message", "text": "【%s】已在公会中。" % mname})
		actions.append(_guild_update_action())
		return {"ok": false, "reason": "already_member", "actions": actions}
	var add_r: Dictionary = guild.try_add_member(mid, mname, Guild.RANK_MEMBER)
	if not bool(add_r.get("ok", false)):
		var r2 := str(add_r.get("reason", ""))
		if r2 == "full":
			actions.append({"type": "system_message", "text": "公会人数已满（最多 %d 人）。" % guild.max_members()})
...
func try_guild_incoming_invite(from_name: String = "旅人甲", guild_name: String = "测试公会") -> Dictionary:
	...
	if guild_name.is_empty():
		guild_name = "测试公会"
	if guild != null and guild.in_guild():
		actions.append({"type": "system_message", "text": "你已经加入公会了。"})
		return {"ok": false, "reason": "already_in_guild", "actions": actions}
	var inv_id := "ginv_%d" % _next_guild_invite_seq
	_next_guild_invite_seq += 1
	var gid := "guild_pending_%d" % _next_guild_seq
	_guild_invites.append({
		"id": inv_id,
		"guild_id": gid,
...
func try_guild_invite_respond(invite_id: String, accept: bool) -> Dictionary:
	...
	var idx := -1
	for i in range(_guild_invites.size()):
		var inv: Variant = _guild_invites[i]
		if typeof(inv) == TYPE_DICTIONARY and str(inv.get("id", "")) == invite_id:
			idx = i
			break
	if idx < 0:
		...
		return {"ok": false, "reason": "not_found", "actions": actions}
	var row: Dictionary = _guild_invites[idx]
	_guild_invites.remove_at(idx)
	...
	if gid.is_empty():
		gid = "guild_%d" % _next_guild_seq
		_next_guild_seq += 1
	var leader_id := str(row.get("from_id", "remote_leader"))
	var leader_name := from_name if from_name != "" else "会长"
	var cr: Dictionary = guild.try_create(gid, gname, leader_id, leader_name)
	...
	var ar: Dictionary = guild.try_add_member(_guild_self_id(), _guild_self_name(), Guild.RANK_MEMBER)
...
func try_guild_kick(member_id: String) -> Dictionary:
	...
	if member_id.is_empty():
		...
		return {"ok": false, "reason": "invalid", "actions": actions}
	if member_id == _guild_self_id() or member_id == guild.leader_id:
		actions.append({"type": "system_message", "text": "不能踢出会长。"})
		return {"ok": false, "reason": "is_leader", "actions": actions}
	var idx: int = guild.find_index_by_id(member_id)
	var mname := member_id
	if idx >= 0:
		var e: Variant = guild.members[idx]
		if typeof(e) == TYPE_DICTIONARY:
			mname = str(e.get("name", member_id))
	var rem: Dictionary = guild.try_remove_member(member_id)
	if not bool(rem.get("ok", false)):
...
func try_guild_leave() -> Dictionary:
	...
	var gname: String = str(guild.guild_name)
	var was_leader: bool = bool(guild.is_leader(self_id))
