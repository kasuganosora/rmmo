...
func create_map(args: Dictionary) -> Dictionary:
	...
	if cw > MAP_MAX_SIDE or ch > MAP_MAX_SIDE:
		return mcp._err("单张地图最大 %d×%d。更大的世界请拆成多张地图，用传送点连接。" % [MAP_MAX_SIDE, MAP_MAX_SIDE])
	...
	if not ok:
		return mcp._err("create_map failed (id exists?)")
	_persist()
	_switch_map(mid)
	return mcp._ok({"map_id": mid, "name": nm, "width": int(doc().width), "height": int(doc().height)})
...
func delete_map(args: Dictionary) -> Dictionary:
	...
	var mid := str(args.get("map_id", "")).strip_edges()
	if mid == "":
		return mcp._err("map_id required")
	if p.map_tree.size() <= 1:
		return mcp._err("cannot delete last map")
	if not bool(p.remove_map(mid)):
		return mcp._err("delete failed")
	_persist()
	if mcp._map_id() == mid:
		...
		if nxt != "":
			_switch_map(nxt)
	if ed() and ed().has_method("_refresh_tree"):
		ed()._refresh_tree()
	return mcp._ok({"deleted": mid, "map_id": mcp._map_id()})
...
func rename_map(args: Dictionary) -> Dictionary:
	...
	if p == null:
		return mcp._err("no pack")
	var mid := str(args.get("map_id", mcp._map_id())).strip_edges()
	var nm := str(args.get("name", "")).strip_edges()
	if mid == "" or nm == "":
		return mcp._err("map_id and name required")
	p.rename_map(mid, nm)
	_persist()
	if ed() and ed().has_method("_refresh_tree"):
		ed()._refresh_tree()
	return mcp._ok({"map_id": mid, "name": nm})
...
func duplicate_map(args: Dictionary) -> Dictionary:
	...
	if p == null:
		return mcp._err("no pack")
	var src := str(args.get("map_id", mcp._map_id())).strip_edges()
	var nid: String = p.duplicate_map(src)
	if nid == "":
		return mcp._err("duplicate failed")
	_persist()
	_switch_map(nid)
	return mcp._ok({"map_id": nid, "from": src})
...
func resize_map(args: Dictionary) -> Dictionary:
	...
	if ed() and ed().has_method("_reload_field"):
		ed()._reload_field()
	return mcp._ok({"width": int(d.width), "height": int(d.height)})
...
func reparent_map(args: Dictionary) -> Dictionary:
	...
	if not bool(p.set_parent(mid, parent)):
		return mcp._err("reparent failed")
	if ed() and ed().has_method("_refresh_tree"):
		ed()._refresh_tree()
	return mcp._ok({"map_id": mid, "parent": parent})
