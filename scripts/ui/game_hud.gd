...
func apply_shop_buyback(rows: Variant) -> void:
	if typeof(rows) != TYPE_ARRAY:
		...
		_shop_buyback = (rows as Array).duplicate(true)
	if _shop_panel != null and _shop_panel.visible:
		_fill_shop_panel()
...
func _add_buyback_row(parent: Node, index: int, label: String, price: int) -> void:
	...
	btn.pressed.connect(func():
		if _world_combat != null and _world_combat.has_method("request_shop_buyback"):
			_world_combat.request_shop_buyback(index)
		else:
			var srv = Net.server()
			if srv != null and srv.has_method("try_shop_buyback"):
				_apply_equip_result_locally(srv.try_shop_buyback(index))
	)
...
func hide_shop() -> void:
	...
	if _world_combat != null and _world_combat.has_method("request_shop_close"):
		_world_combat.request_shop_close()
	else:
		var srv = Net.server()
...
func _ensure_shop_panel() -> void:
	if _shop_panel != null and is_instance_valid(_shop_panel):
		if bool(_shop_panel.get_meta("shop_v4", false)):
			return
...
func _make_shop_list_pane(list_name: String, heading: String) -> PanelContainer:
	...
	pane.add_child(col)
	_add_label(col, heading, 11, L2Style.COL_MUTED)
	var scroll := ScrollContainer.new()
...
func _on_shop_tab(tab_id: String) -> void:
	...
	_show_shop_tab_pages()
	if _shop_panel != null:
		_highlight_shop_tabs(_shop_panel.get_meta("shop_tabs", null) as HBoxContainer)
...
func _highlight_shop_tabs(tabs: HBoxContainer) -> void:
	...
	for i in range(tabs.get_child_count()):
		...
		if btn == null or i >= SHOP_TABS.size():
			continue
		L2Style.style_tab_button(btn, str(SHOP_TABS[i][1]) == _shop_tab)


func _show_shop_tab_pages() -> void:
	if _shop_panel == null:
		return
	var buy := _shop_panel.find_child("BuyPage", true, false) as Control
	var sell := _shop_panel.find_child("SellPage", true, false) as Control
	if buy != null:
		buy.visible = _shop_tab == "buy"
	if sell != null:
		sell.visible = _shop_tab == "sell"
	var back := _shop_panel.find_child("BuybackPage", true, false) as Control
	if back != null:
		back.visible = _shop_tab == "buyback"
...
func _place_shop_panel() -> void:
	if _shop_panel == null:
		return
	var vp := get_viewport().get_visible_rect().size
	_shop_panel.size = Vector2(580, 460)
...
func _fill_shop_panel() -> void:
	...
	if title_l != null:
		if _shop_vendor_rep >= 0:
			title_l.text = "%s  声望 %d" % [_shop_title if _shop_title != "" else "商店", _shop_vendor_rep]
		else:
			title_l.text = _shop_title if _shop_title != "" else "商店"
	var gold_l := _shop_panel.get_meta("gold_label") as Label
	if gold_l != null:
		gold_l.text = "Adena  %d" % maxi(_server_gold, 0)
		L2Style.style_gold_amount(gold_l)
	var rep_l := _shop_panel.find_child("ShopRep", true, false) as Label
	if rep_l != null:
		if _shop_vendor_rep >= 0:
			rep_l.text = "声望 %d" % _shop_vendor_rep
			rep_l.visible = true
		else:
			rep_l.text = ""
			rep_l.visible = false
	var footer_rep := _shop_panel.find_child("ShopRepFooter", true, false) as Label
	if footer_rep != null:
		if _shop_vendor_rep >= 0:
			footer_rep.text = "声望 %d" % _shop_vendor_rep
	...
	if buy_cat != null:
		if _shop_listings.is_empty():
			_add_label(buy_cat, "（无商品）", 12, L2Style.COL_MUTED)
		else:
	...
	if sell_cat != null:
		...
		if sellables.is_empty():
			_add_label(sell_cat, "（无可出售物品）", 12, L2Style.COL_MUTED)
		else:
	...
	var back_cat := _shop_panel.find_child("BuybackCatalog", true, false) as VBoxContainer
	_clear_container(back_cat)
	if back_cat != null:
		if _shop_buyback.is_empty():
			_add_label(back_cat, "（无回购）", 12, L2Style.COL_MUTED)
		else:
			for i in range(_shop_buyback.size()):
				var row: Variant = _shop_buyback[i]
...
func _add_shop_catalog_row(parent: VBoxContainer, label_text: String, price: int, is_buy: bool, item_id: String, display_name: String, unit_price: int) -> void:
	...
	btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	btn.tooltip_text = _equip_compare_tip(item_id, "%s\n%d Adena\n点击加入选择" % [_item_rarity_name_line(item_id, display_name), unit_price])
	L2Style.style_row_button(btn, false)
...
func _fill_cart_list(parent: VBoxContainer, cart: Array, is_buy: bool) -> void:
	if parent == null:
		return
	if cart.is_empty():
		_add_label(parent, "（未选择）", 12, L2Style.COL_MUTED)
		return
	var total := 0
	for line in cart:
		...
		row.add_child(minus)
	_add_label(parent, "合计  %d Adena" % total, 12, L2Style.COL_GOLD)
...
func _on_shop_confirm() -> void:
	...
	if _shop_tab == "sell":
		_confirm_sell_cart()
	elif _shop_tab == "buyback":
		append_system("点选回购列表中的物品即可买回。")
	else:
		_confirm_buy_cart()
...
func _on_shop_sell_junk() -> void:
	if _world_combat != null and _world_combat.has_method("request_shop_sell_junk"):
		_world_combat.request_shop_sell_junk()
		return
	var srv = Net.server()
