...
static func show_shop(ctrl, shop_id: String, title: String, listings: Array, gold: int = 0, vendor_rep: int = -1) -> void:
...
static func _add_buyback_row(ctrl, parent: Node, index: int, label: String, price: int) -> void:
	var btn = Button.new()
...
static func hide_shop(ctrl) -> void:
	if ctrl._shop_panel != null:
		ctrl._shop_panel.visible = false
	ctrl._shop_id = ""
	ctrl._shop_vendor_rep = -1
	ctrl._shop_buyback = []
	ctrl._clear_shop_carts()
...
static func _clear_shop_carts(ctrl) -> void:
	ctrl._shop_buy_cart.clear()
	ctrl._shop_sell_cart.clear()

static func _ensure_shop_panel(ctrl) -> void:
	...
	close_btn.pressed.connect(ctrl.hide_shop)
	...
	vbox.add_child(tabs)
	var buy_root = ctrl._make_shop_tab_page("BuyPage", "BuyCatalog", "BuyCart", "商品列表", "我的选择")
	vbox.add_child(buy_root)
	var sell_root = ctrl._make_shop_tab_page("SellPage", "SellCatalog", "SellCart", "背包可出售", "我的选择")
	vbox.add_child(sell_root)
	var back_root = ctrl._make_shop_tab_page("BuybackPage", "BuybackCatalog", "BuybackCart", "最近卖出", "回购")
	...
	junk_btn.pressed.connect(ctrl._on_shop_sell_junk)
	L2Style.style_action_button(junk_btn)
	bottom.add_child(junk_btn)
	var cancel_btn = Button.new()
	cancel_btn.text = "取消"
	cancel_btn.focus_mode = Control.FOCUS_NONE
	cancel_btn.pressed.connect(ctrl.hide_shop)
	L2Style.style_action_button(cancel_btn)
	bottom.add_child(cancel_btn)
	var ok_btn = Button.new()
	ok_btn.text = "确定"
	ok_btn.focus_mode = Control.FOCUS_NONE
	ok_btn.pressed.connect(ctrl._on_shop_confirm)
	L2Style.style_action_button(ok_btn)
	bottom.add_child(ok_btn)
	panel.set_meta("shop_tabs", tabs)
	ctrl._apply_l2_chrome(panel)
	ctrl._rebuild_shop_tab_bar(panel)
	ctrl._show_shop_tab_pages()
	panel.set_meta("tabs", tabs)
	panel.set_meta("gold_label", gold_l)
	ctrl._shop_panel = panel
	ctrl.call_deferred("_place_shop_panel")

static func _make_shop_tab_page(ctrl, page_name: String, catalog_name: String, cart_name: String, cat_title: String, cart_title: String) -> HBoxContainer:
	var root = HBoxContainer.new()
	root.name = page_name
	root.add_theme_constant_override("separation", 8)
	root.size_flags_vertical = Control.SIZE_EXPAND_FILL
	root.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var left = ctrl._make_shop_list_pane(catalog_name, cat_title)
	left.size_flags_stretch_ratio = 1.35
	root.add_child(left)
	var right = ctrl._make_shop_list_pane(cart_name, cart_title)
	right.size_flags_stretch_ratio = 1.0
	root.add_child(right)
	return root

static func _make_shop_list_pane(ctrl, list_name: String, heading: String) -> PanelContainer:
...
static func _rebuild_shop_tab_bar(ctrl, panel: PanelContainer) -> void:
	...
	for item in SHOP_TABS:
		...
		btn.pressed.connect(ctrl._on_shop_tab.bind(str(item[1])))
		tabs.add_child(btn)
	ctrl._highlight_shop_tabs(tabs)

static func _on_shop_tab(ctrl, tab_id: String) -> void:
	...
	ctrl._show_shop_tab_pages()
	if ctrl._shop_panel != null:
		ctrl._highlight_shop_tabs(ctrl._shop_panel.get_meta("shop_tabs", null) as HBoxContainer)

static func _highlight_shop_tabs(ctrl, tabs: HBoxContainer) -> void:
	if tabs == null:
		return
...
static func _show_shop_tab_pages(ctrl) -> void:
	if ctrl._shop_panel == null:
		return
...
static func _place_shop_panel(ctrl) -> void:
	if ctrl._shop_panel == null:
		return
...
static func _fill_shop_panel(ctrl) -> void:
	ctrl._ensure_shop_panel()
	if ctrl._shop_panel == null:
		return
	...
	if buy_cat != null:
		if ctrl._shop_listings.is_empty():
			...
			for it in ctrl._shop_listings:
				...
				ctrl._add_shop_catalog_row(buy_cat, iname, price, true, iid, iname, price)
	# Buy cart
	ctrl._fill_cart_list(buy_cart, ctrl._shop_buy_cart, true)
	# Sell catalog from bag
	if sell_cat != null:
		var sellables: Array = ctrl._shop_sellable_bag_rows()
		if sellables.is_empty():
			...
			for it2 in sellables:
				...
				ctrl._add_shop_catalog_row(sell_cat, "%s×%d" % [iname2, have_q], sprice, false, iid2, iname2, sprice)
	ctrl._fill_cart_list(sell_cart, ctrl._shop_sell_cart, false)
	...
	if back_cat != null:
		if ctrl._shop_buyback.is_empty():
			...
			for i in range(ctrl._shop_buyback.size()):
				...
				ctrl._add_buyback_row(back_cat, i, "%s×%d" % [nm3, q3], p3)

static func _add_shop_catalog_row(ctrl, parent: VBoxContainer, label_text: String, price: int, is_buy: bool, item_id: String, display_name: String, unit_price: int) -> void:
	...
	L2Style.style_row_button(btn, false)
	btn.pressed.connect(ctrl._on_shop_catalog_add.bind(is_buy, item_id, display_name, unit_price))
	row.add_child(btn)
	...
	add_btn.pressed.connect(ctrl._on_shop_catalog_add.bind(is_buy, item_id, display_name, unit_price))
	row.add_child(add_btn)

static func _fill_cart_list(ctrl, parent: VBoxContainer, cart: Array, is_buy: bool) -> void:
	if parent == null:
		return
	if cart.is_empty():
	...
	for line in cart:
		...
		lab.pressed.connect(ctrl._on_shop_cart_remove.bind(is_buy, iid))
		row.add_child(lab)
		...
		minus.pressed.connect(ctrl._on_shop_cart_adjust.bind(is_buy, iid, -1))
		row.add_child(minus)
	ctrl._add_label(parent, "合计  %d Adena" % total, 12, L2Style.COL_GOLD)

static func _on_shop_catalog_add(ctrl, is_buy: bool, item_id: String, display_name: String, unit_price: int) -> void:
...
static func _on_shop_cart_adjust(ctrl, is_buy: bool, item_id: String, delta: int) -> void:
	item_id = item_id.strip_edges()
	var cart: Array = ctrl._shop_buy_cart if is_buy else ctrl._shop_sell_cart
	for i in range(cart.size()):
		var line: Dictionary = cart[i]
...
static func _on_shop_cart_remove(ctrl, is_buy: bool, item_id: String) -> void:
	item_id = item_id.strip_edges()
	var cart: Array = ctrl._shop_buy_cart if is_buy else ctrl._shop_sell_cart
...
static func _on_shop_confirm(ctrl) -> void:
	if ctrl._shop_panel == null:
		return
	if ctrl._shop_tab == "sell":
		ctrl._confirm_sell_cart()
	elif ctrl._shop_tab == "buyback":
		ctrl.append_system("点选回购列表中的物品即可买回。")
	else:
		ctrl._confirm_buy_cart()

static func _confirm_buy_cart(ctrl) -> void:
...
static func _confirm_sell_cart(ctrl) -> void:
...
static func _shop_sellable_bag_rows(ctrl) -> Array:
	var out: Array = []
	var cat = null
	var srv = Net.server()
	if srv != null:
...
static func _on_shop_buy(ctrl, item_id: String) -> void:
...
static func _on_shop_sell(ctrl, item_id: String) -> void:
...
static func _on_shop_sell_junk(ctrl) -> void:
