...
func _run() -> void:
	...
	failed += _expect(bool(tp.collision.is_landable(2, 2)), "starter grass landable")
	failed += _expect(not bool(tp.collision.is_landable(1, 1)), "erased void not landable")
	...
	failed += _expect(not bool(sv.map_collision.is_landable(1, 1)), "server erased void not landable")
	sv.queue_free()
	if failed == 0:
		print("ALL PASS")
		quit(0)
